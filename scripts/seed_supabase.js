...
const defaultCategories = ['Technology', 'Design', 'Lifestyle', 'Discussion'];
...
async function seedSupabase() {
  ...
  console.log('👤 Setting up Admin / Author User...');
  ...
  if (!admin) {
    ...
    admin = await prisma.user.create({
      data: {
        ...
        role: 'ADMIN',
        bio: 'Chief Editor & Lead Systems Architect curating timeless stories on design, engineering, and mindful living.',
        image: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80'
      }
    });
    console.log('  + Created Admin User: admin@example.com (Password: Admin@12345)');
  } else {
    console.log(`  ✓ Using existing Admin User: ${admin.email}`);
  }

  // 3. Populate 8 Rich Articles
  console.log('📝 Seeding rich editorial articles...');
  for (const postData of seedPosts) {
    const categoryId = catMap[postData.categoryName];
    if (!categoryId) {
      console.warn(`  ! Category not found for: ${postData.categoryName}`);
      continue;
    }
